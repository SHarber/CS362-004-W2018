...
int main(int argc, char** argv)
{
	...
	int seed = 66;				// Seed for Game Initialization
	...
	printf("Deck Count: %d \n", thisGame.deckCount[0]);
	for (i = 0; i < thisGame.deckCount[0]; i++)
	{
		printf("Card %d: %d\n", i, thisGame.deck[0][i]);
	}
	printf("Discard Count: %d \n", thisGame.discardCount[0]);
	for (i = 0; i < thisGame.discardCount[0]; i++)
	{
		printf("Discard Card: %d: %d\n", i, thisGame.discard[0][i]);
	}
	printf("Hand Count: %d \n", thisGame.handCount[0]);
	for (i = 0; i < thisGame.handCount[0]; i++)
	{
